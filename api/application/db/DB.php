...
class DB {
    ...
    public function getPetAllInfo($id) {    
        $query = 'SELECT p.*, c.number AS cageNumber, sp.name as species, gen.name as genus, ord.name as order_, cl.name as class, ph.imgName as img
                FROM pets AS p 
                LEFT JOIN cages AS c ON p.cageId = c.id  
                LEFT JOIN petPhotos AS ph ON ph.petId = p.id
    ...
    public function getPetbySpecies($species) {
        $query = 'SELECT p.*, c.number AS cageNumber, sp.name AS species 
                ...
                LEFT JOIN cages AS c ON p.cageId = c.id  
                LEFT JOIN animal_species as sp ON p.speciesId = sp.id
                WHERE p.isAlive = 1 AND p.speciesId = ' . $species;
        return $this->getArray($query);         
    }
    ...
    public function getPetbyCage($cage) {
        $query = 'SELECT p.*, sp.name AS species 
                FROM pets AS p  
                LEFT JOIN animal_species as sp ON p.speciesId = sp.id
                WHERE p.isAlive = 1 AND p.cageId = ' . $cage;
        return $this->getArray($query);         
    }    
